...
<body>
	...
	<c:if test="${sessionScope.user.isAdmin()==true }">
				...
				<div class="grid">
					<div class="add-product">
						<div style="font-size: 1.5rem; margin-bottom: 20px;">
							<a href="index.jsp" style="padding-right: 10px;">Trang chủ</a><span>></span><a
								href="admin.jsp" style="padding: 0 10px;">Admin</a><span>></span><span
								style="padding-left: 10px;">Sửa sản phẩm</span>
						</div>
						<form class="add-form " action="editControl" method="post"
							enctype="multipart/form-data">
							<input class="hide" name="id" value="${product.getId() }">
							<input class="hide" name="image" value="${product.getImage() }">
							<div class="add-name">
								<label>Nhập tên sản phẩm</label> <input type="text" class=""
									value="${product.getName() }" name="name">
							</div>
							<div class="add-name">
								<label>Tiêu đề sản phẩm</label> <input type="text" class=""
							...
							<div class="add-name">
								<img alt="" src="${product.getImage() }" width="100px"
									height="100px"> <label>Thay ảnh</label> <input
									type="file" name="photo">
							</div>
							<div class="add-name">
								<label>Giá bán</label> <input type="text" class="num"
									value="${product.getPrice() }" name="price">
								<p class="err hide err-num">Giá nhập không hợp lệ</p>
							</div>
							<div class="add-name">
								<label>Giá bìa</label> <input type="text" class="num"
									name="publishedPrice" value="${product.getPublishedPrice() }">
								<p class="err hide err-num">Giá nhập không hợp lệ</p>
							</div>
							<div class="add-name">
								<label>Số lượng</label> <input type="text" class="num"
									value=${product.getAmount() } name="amount">
								<p class="err hide err-num">Số lượng không hợp lệ</p>
							</div>
							<div class="add-name">
								<label>Nhà xuất bản</label> <input type="text" class=""
									value="${product.getPublished() }" name="published">
							</div>
							<div class="add-name">
								<label>Tác giả</label> <input type="text" class="" name="author"
									value="${product.getAuthor() }">
							</div>
							<div class="add-name">
								<label>Thể loại</label> <select class="" name="nameCate">
									<c:forEach items="${category.getListCategories()}" var="cate">
							...
							<div class="add-name">
								<label>Phân loại sản phẩm</label> <select class=""
									name="nameClassify">
        ...
        <a href="index.jsp" style="font-size:1.5rem;color:blue;">Quay lại trang chủ</a>
